...
class ClassicChkPtAgentTest : public ::testing::Test {
    ...
    ClassicChkPtAgentTest() {
        dataTypeInator = new DataTypeInator;
...
TEST_F(ClassicChkPtAgentTest, writeDeclaration1) {
    ...
    AllocInfo * allocInfo;
    try {
        allocInfo = new AllocInfo("A", "int", dataTypeInator);
...
TEST_F(ClassicChkPtAgentTest, writeDeclaration3) {
    ...
    try {
        // int dimSize[] = {3,-1};
        allocInfo = new AllocInfo("A", "int *[3]", dataTypeInator);
    } catch ( const std::logic_error& e ) {
        std::cerr << e.what();
        construction_result = false;
    }
...
TEST_F(ClassicChkPtAgentTest, writeDeclaration5) {
    ...
    AllocInfo * allocInfo;
    try {
        allocInfo = new AllocInfo("A", "int*(*[3][4])[5]", dataTypeInator);
    } catch ( const std::logic_error& e ) {
        std::cerr << e.what();
        construction_result = false;
    }
...
TEST_F(ClassicChkPtAgentTest, writeDeclaration_DayOfWeekEnum) {
    ...
    std::stringstream output_declaration;
    bool construction_result = true;
    bool result = addDayOfWeekEnumToTypeDictionary( dataTypeInator, enumDictionary );
    EXPECT_EQ(true, result);
    AllocInfo * allocInfo;
    try {
        allocInfo = new AllocInfo("A", "DayOfWeek", dataTypeInator);
    } catch ( const std::logic_error& e ) {
        std::cerr << e.what();
        construction_result = false;
    }
...
TEST_F(ClassicChkPtAgentTest, getSubName1) {
    ...
    bool result = addClassOneToTypeDictionary( dataTypeInator );
    EXPECT_EQ(true, result);

    const DataType* classOneDataType = dataTypeInator->resolve("ClassOne");
    const DataType* intDataType = dataTypeInator->resolve("int");
    const DataType* doubleDataType = dataTypeInator->resolve("double");
...
TEST_F(ClassicChkPtAgentTest, getSubName2) {
    ...
    result = addClassOneToTypeDictionary( dataTypeInator );
    EXPECT_EQ(true, result);

    result = addClassTwoToTypeDictionary( dataTypeInator );
    EXPECT_EQ(true, result);
...
TEST_F(ClassicChkPtAgentTest, getSubName3) {
    ...
    bool result = addClassThreeToTypeDictionary( dataTypeInator );
    EXPECT_EQ(true, result);

    const DataType* classThreeDataType = dataTypeInator->resolve("ClassThree");
    const DataType* doubleDataType = dataTypeInator->resolve("double");
...
TEST_F(ClassicChkPtAgentTest, writeVariable1) {
    ...
    AllocInfo * allocInfo;
    try {
        allocInfo = new AllocInfo("A", "ClassOne", dataTypeInator);
    } catch ( const std::logic_error& e ) {
        std::cerr << e.what();
        construction_result = false;
    }
...
TEST_F(ClassicChkPtAgentTest, writeVariable3) {
    ...
    bool result;
    result = addClassThreeToTypeDictionary( dataTypeInator );
    EXPECT_EQ(true, result);
