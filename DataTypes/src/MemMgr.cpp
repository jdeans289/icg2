...
void* MemMgr::declare_var( const std::string& typeSpecifier,
    ...
    try {
        ...
        DeclarationBuilder builder (typeSpecifier, dims);
        newAllocInfo = new AllocInfo(variableName, builder.getAbstractDeclarator(), dataTypeInator, suppliedAllocation);
...
void* MemMgr::declare_var( const std::string& declaration,
    ...
    // std::cout << "Parsed type specifier: " << typeSpecifier << std::endl;
    // std::cout << "Parsed variable name: " << variableName << std::endl;


    unsigned int dimensionsCount = typeName->getNumberOfDimensions();
    int dimensions[16];
    for (unsigned int ii=0 ; ii < dimensionsCount; ii++) {
